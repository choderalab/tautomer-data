...
def plot_two_distributions(x_list: list, y_list: list):
    ...
    plt.xlabel("$\Delta_{t}G_{solv}$ [kcal/mol]", fontsize=fontsize)
    ...
    sns.distplot(
        ...
        rug=True,
        bins=15,
        label="$\Delta_{t}G_{solv}^{exp} - \Delta_{t}G_{solv}^{calc}$",
    )
    plt.xlabel(
        "$\Delta_{t}G_{solv}^{exp} - \Delta_{t}G_{solv}^{calc}$ [kcal/mol]",
        fontsize=fontsize,
    )
    plt.ylabel("Probability", fontsize=fontsize)
    plt.axvline(0, 0, 15, color="red")
...
def plot_diff(x_list: list, y_list: list):
    ...
    plt.xlabel(
        "$\Delta_{t} G_{solv}^{exp} - \Delta_{t} G_{solv}^{calc}$ [kcal/mol]", fontsize=fontsize
    )
    plt.ylabel("Probability", fontsize=fontsize)
    plt.axvline(0, 0, 15, color="red")
    kl = compute_kl_divergence(np.array(x_list), np.array(y_list))
